...
class SkipGram:
    ...
    def backward(self, target_idx, context_indices):
        e = self.y_pred.copy()
        for idx in context_indices:
            e[0][idx] -= 1  # error term for each context word
        ...
        # update W1
        grad_w1 = (self.lr * dh.reshape(-1)) / len(context_indices)
        self.W1[target_idx] -= grad_w1

        loss = -np.sum([np.log(self.y_pred[0][idx] + 1e-10) for idx in context_indices])  # add small value to prevent log(0)
        return loss